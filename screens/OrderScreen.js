...
const OrderScreen = () => {
  ...
  const fetchOrderByUserId = async () => {
    const res = await fetch(
      `https:enblapayment.onrender.com/api/order/getOrder/${userId}`
    );
  ...
  return (
    <ScrollView className="bg-white min-h-screen pt-16 px-2 ">
      <SafeAreaView className="pb-20">
        <View className="">
          <Text className="font-medium text-left text-3xl">
            Your Order History
          </Text>
          ...
          <View className="flex-row items-center space-x-2 pb-7 pt-3">
            <View className="flex-row space-x-2 flex-1 bg-gray-200 px-3 py-1 items-center rounded-xl border-cyan-700 ">
              ...
              <Text className="w-[100%] py-3 rounded-sm text-lg bg-slate-100 text-center font-medium outline-none border-none ">
                Your Orders
              </Text>
        ...
        <View className="mt-4">
          {orders?.length === 0 ? (
            <Text style={{ textAlign: "center" }}>You Have No Order</Text>
          ) : (
            orders?.map((order) => (
              <OrderCard
                key={order._id}
