...
import { select_basket_total, select_basket_item, remove_all_items } from '../redux/basketSlice'
...
const BottomUpPopup = () => {
    ...
    const {user:{address, userId, firstName, lastName, email}} = useSelector(state => state.user)
    ...
    useEffect(() => {
      ...
      const payWithChapa = async () => {
        ...
        const res = await fetch('https://enblapayment.onrender.com/api/payment/chapa', options )
            ...
            <Animatable.View 
                ...
                <View className="">
                    <View className="flex flex-row justify-between">
                        ...
                        <Text className="text-xl font-medium text-left">~ETB {Math.floor(-total_basket) + Math.floor((-total_basket)/16)}</Text>
                    </View>
                    <Text className="w-[328px] text-left text-sm text-gray-600">
                        Please note that chapa is the only payment option we have so far and more will be added 
                        soon
                    </Text>
                </View>
                <View className="border-y border-gray-300/80 my-4 rounded"/>
